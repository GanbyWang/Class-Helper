...
public class MyVote extends AppCompatActivity {
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            // System.out.println("MyVote: " + msg.obj.toString());
                ...
                case HttpGet.GET_SUCC:
                    try {
                        ...
                        for (int i = 0; i < jsons.length(); i++) {
                            ...
                            map.put("link", jsonInfo.getString("link"));
                            map.put("id", jsonInfo.getString("id"));
                            map.put("author", jsonInfo.getString("author"));
                            myData.add(map);
                        }
                        vote_list = (ListView) findViewById(R.id.vote_list);
                        MyVote.MyAdapter adapter = new MyVote.MyAdapter(MyVote.this);
                        vote_list.setAdapter(adapter);
                    } catch (JSONException e) {
                        e.printStackTrace();
                        System.out.println("MyVote: JSON WRONG!!");
    ...
    public class MyAdapter extends BaseAdapter {
        ...
        public void deleteVote(int position) {
            new HttpDelete("http://101.200.163.140:3000/api/customers/" + LogInActivity.userId +
                    "/votes/" + myData.get(position).get("id").toString() +
                    "?access_token=" + LogInActivity.access_token,
                    handler, HttpDelete.TYPE_VOTE, null);
        }
    }
}
